...
| `?outline=sticker`、`?rim=1`、`?shadow=1` | 白フチ、リムライト、影を表示します。 |
| `?aura=1`、`?aura=flame` | 背後にグローのオーラ、または炎のオーラを表示します。 |
| `?silcache=0` | シルエットのキャッシュを使わず、影、オーラ、輪郭の輪を元の絵から毎回描きます（重くなります。見た目の比較用です）。 |
| `?echo=0.6` | 声の大きさに合わせて広がる輪郭の輪を、音量0〜1の固定値で表示します。 |
